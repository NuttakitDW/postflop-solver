...
fn main() {
    ...
    // Card config - starting from flop
    ...
    // Bet sizes from your config: 25%, 50%, 100% bet, 50% raise
    let bet_sizes = BetSizeOptions::try_from(("25%, 50%, 100%", "50%")).unwrap();

    // Tree config from your configuration.json
    ...
    // Allocate memory - false = 32-bit FP (no compression)
    ...
    let target_14cpu = 135.0; // 2:15
    let target_32cpu = 60.0;  // 1:00
    let actual = solve_time.as_secs_f64();

    println!("Target (14 CPU): {:.0}s (2:15)", target_14cpu);
    println!("Target (32 CPU): {:.0}s (1:00)", target_32cpu);
    println!("Actual time: {:.2}s", actual);
    println!();

    if actual <= target_14cpu {
        println!("✓ PASS: Within 14 CPU target!");
        println!("  Projected 32 CPU time: {:.0}s", actual * 14.0 / 32.0);
    } else {
        println!("✗ FAIL: {:.1}x slower than target", actual / target_14cpu);
        println!("  Need {:.1}x speedup to meet target", actual / target_14cpu);
    }
}
