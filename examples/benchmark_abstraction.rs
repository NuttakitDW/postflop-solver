...
fn main() {
    ...
    // Same solve parameters as baseline
    let max_iterations = 1000;
    let target_exploitability = game.tree_config().starting_pot as f32 * 0.005; // 0.5% of pot

    println!("=== Solving ===");
    println!("Max iterations: {}", max_iterations);
    println!("Target exploitability: {:.4} (0.5% of pot)", target_exploitability);
    println!();
    ...
    let baseline_time = 84.38; // From baseline_results.txt
    let baseline_exploitability = 0.2477;
    let baseline_memory = 11728.68; // MB

    println!("                    Baseline    Abstracted    Change");
    println!("Solve time:         {:.2}s       {:.2}s         {:.1}x",
             baseline_time, solve_time.as_secs_f64(),
             baseline_time / solve_time.as_secs_f64());
    println!("Exploitability:     {:.4}      {:.4}        {:.1}x",
             baseline_exploitability, exploitability,
             exploitability / baseline_exploitability);
    println!("Memory:             {:.2} MB   {:.2} MB     -{:.1}%",
             baseline_memory, mem_uncompressed_abs as f64 / 1024.0 / 1024.0,
             memory_reduction);
    println!();

    // Summary
    println!("=== Summary ===");
    if solve_time.as_secs_f64() < baseline_time {
        println!("Speed: {:.1}x FASTER", baseline_time / solve_time.as_secs_f64());
    } else {
        println!("Speed: {:.1}x slower", solve_time.as_secs_f64() / baseline_time);
    }
    println!("Memory: {:.1}% reduction", memory_reduction);
    println!("Exploitability ratio: {:.2}x baseline", exploitability / baseline_exploitability);
}
