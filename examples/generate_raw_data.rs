...
// ---------------------------------------------------------------------------
// Per-sample processing: one board = one data point (RAW, no bucketing)
// ---------------------------------------------------------------------------
...
fn process_sample(
    ...
    // Assemble raw output — NO bucketing
...
fn parse_args() -> (String, usize, f32, u64) {
    ...
    let mut output_dir = String::from("./data/solver_output");
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--help" | "-h" => {
                ...
                eprintln!(
                    "  --output-dir <DIR>       Output directory (default: ./data/solver_output)"
                );
                eprintln!(
...
fn main() {
    ...
    eprintln!("=== Raw Training Data Generator ===");
    ...
    let results: Vec<RawSampleResult> = (0..num_samples)
        .into_par_iter()
        .filter_map(|i| {
