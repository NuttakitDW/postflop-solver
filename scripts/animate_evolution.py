...
Shows all boundaries as thin lines + bold mean line across all boundaries.

Usage:
    python scripts/animate_evolution.py data/bt1/KcQh7s.bt1
"""
...
def animate(data, output_path, board_name):
    n = data["num_iterations"]
    nb = data["num_boundaries"]

    # Per-boundary mean across hands: [boundary, iteration]
    mean_cr_ip = data["cfreach_ip"].mean(axis=2)   # [nb, n]
    mean_cr_oop = data["cfreach_oop"].mean(axis=2)  # [nb, n]
    mean_cfv_oop = data["cfv_oop"].mean(axis=2)     # [nb, n]
    mean_cfv_ip = data["cfv_ip"].mean(axis=2)       # [nb, n]

    # Grand mean across all boundaries: [iteration]
    ...
    # Colors for per-boundary lines
    cmap_blue = plt.cm.Blues(np.linspace(0.3, 0.8, nb))
    cmap_green = plt.cm.Greens(np.linspace(0.3, 0.8, nb))
    cmap_blue_cfv = plt.cm.Blues(np.linspace(0.3, 0.8, nb))
    cmap_green_cfv = plt.cm.Greens(np.linspace(0.3, 0.8, nb))

    # ---- cfreach panel: per-boundary thin lines + bold mean ----
    ...
    # ---- CFV panel: per-boundary thin lines + bold mean ----
    boundary_cfv_oop_lines = []
    boundary_cfv_ip_lines = []
    for b in range(nb):
        l_oop, = ax_cfv.plot([], [], color=cmap_blue_cfv[b], linewidth=0.5, alpha=0.3)
        l_ip, = ax_cfv.plot([], [], color=cmap_green_cfv[b], linewidth=0.5, alpha=0.3)
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python scripts/animate_evolution.py <file.bt1>")
        sys.exit(1)
