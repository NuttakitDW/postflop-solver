...
DATA_DIR = os.path.join(BASE, "..", "data",   "toy_20bb_training")
OUT_DIR  = os.path.join(BASE, "..", "models", "toy_20bb")
...
def main():
    ...
    # Random sample-level split (not board-grouped) for toy game.
    # With only 49 unique turn boards, board-grouped split would exclude ~9
    # entire turn cards from training, making the model predict garbage for them.
    rng_split = np.random.RandomState(RNG_SEED)
