...
mod tests {
    ...
    #[test]
    fn save_and_load_file_with_abstraction() {
        ...
        let card_config = CardConfig {
            ...
            turn: card_from_str("3d").unwrap(),
            river: card_from_str("9s").unwrap(),
        };
        ...
        let tree_config = TreeConfig {
            ...
            starting_pot: 100,
            effective_stack: 100,
            flop_bet_sizes: [Default::default(), Default::default()],
            turn_bet_sizes: [Default::default(), Default::default()],
        ...
        game.enable_abstraction(&abs_config).unwrap();

        game.allocate_memory(false);
        solve(&mut game, 100, 0.0, false);

        // Verify abstraction is enabled before save
        assert!(game.is_abstraction_enabled());
        let num_buckets_before = [
            game.abstraction_data().unwrap().num_buckets(0),
            game.abstraction_data().unwrap().num_buckets(1),
        ];

        // Get strategy before save
        game.cache_normalized_weights();
        let strategy_before = game.strategy();
        let ev_before = game.expected_values(0);

        // save
        save_data_to_file(&game, "abstraction test", "tmpfile-abs.flop", None).unwrap();

        // load
        let (mut loaded_game, memo): (PostFlopGame, String) =
            load_data_from_file("tmpfile-abs.flop", None).unwrap();

        // remove tmpfile
        std::fs::remove_file("tmpfile-abs.flop").unwrap();

        // Verify abstraction is enabled after load
        assert!(loaded_game.is_abstraction_enabled());
        assert_eq!(memo, "abstraction test");

        // Verify bucket counts match
        let num_buckets_after = [
            loaded_game.abstraction_data().unwrap().num_buckets(0),
            loaded_game.abstraction_data().unwrap().num_buckets(1),
        ];
        assert_eq!(num_buckets_before, num_buckets_after);

        // Verify strategy matches
        loaded_game.cache_normalized_weights();
        let strategy_after = loaded_game.strategy();
        let ev_after = loaded_game.expected_values(0);

        assert_eq!(strategy_before.len(), strategy_after.len());
        for (before, after) in strategy_before.iter().zip(strategy_after.iter()) {
            assert!((before - after).abs() < 1e-5,
                "Strategy mismatch: before={}, after={}", before, after);
        }

        assert_eq!(ev_before.len(), ev_after.len());
        for (before, after) in ev_before.iter().zip(ev_after.iter()) {
            assert!((before - after).abs() < 1e-4,
                "EV mismatch: before={}, after={}", before, after);
        }
    }
}
