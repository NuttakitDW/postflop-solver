...
/// Performs Discounted CFR algorithm using the two-reach traversal for neural network integration.
///
/// Tracks both players' reach probabilities through the tree. Without a neural network,
/// this produces identical results to `solve()` since `player_reach` is carried but unused.
///
/// Returns the exploitability of the obtained strategy.
pub fn solve_with_nn(
    ...
    for t in 0..max_num_iterations {
        ...
        // power-of-4 exploitability recalculation (matches solve())
        let is_power_of_4 = t > 0 && t == 1u32 << ((t.leading_zeros() ^ 31) & !1);
        if is_power_of_4 {
            exploitability = compute_exploitability(game);
        }

        // convergence mode (matches solve())
        let current_percent = exploitability / starting_pot * 100.0;
        if starting_pot > 0.0 && current_percent < 1.0 {
            convergence_mode = true;
        }
        ...
        // alternating updates — same as standard solve()
        for player in 0..2 {
            let mut result = Vec::with_capacity(game.num_private_hands(player));
...
/// Recursively solves counterfactual values, tracking both players' reaches.
///
/// Identical to `solve_recursive()` but carries `player_reach` (the updating player's
/// reach probabilities) through the tree alongside `cfreach` (opponent's reach).
/// Without a neural network at chance nodes, `player_reach` is unused and results
/// are bit-exact identical to `solve_recursive()`.
fn solve_recursive_with_nn(
    ...
    if num_actions == 1 && !node.is_chance() {
        let child = &mut node.play(0);
        solve_recursive_with_nn(result, game, child, player, cfreach, player_reach, params);
        return;
    }
    ...
    if node.is_chance() {
        ...
        // scale both reaches by 1/chance_factor
        #[cfg(feature = "custom-alloc")]
        let mut cfreach_updated = Vec::with_capacity_in(cfreach.len(), StackAlloc);
        #[cfg(not(feature = "custom-alloc"))]
        let mut cfreach_updated = Vec::with_capacity(cfreach.len());
        ...
        let mut player_reach_updated = Vec::with_capacity(player_reach.len());
        mul_slice_scalar_uninit(
            player_reach_updated.spare_capacity_mut(),
            player_reach,
            1.0 / chance_factor as f32,
        );
        unsafe { player_reach_updated.set_len(player_reach.len()) };

        // recurse into each chance outcome
        for_each_child(node, |action| {
            solve_recursive_with_nn(
                row_mut(cfv_actions.lock().spare_capacity_mut(), action, num_hands),
                game,
                ...
                player,
                &cfreach_updated,
                &player_reach_updated,
                params,
            );
        });

        // sum CFVs in f64 precision
        #[cfg(feature = "custom-alloc")]
        let mut result_f64 = Vec::with_capacity_in(num_hands, StackAlloc);
        #[cfg(not(feature = "custom-alloc"))]
        let mut result_f64 = Vec::with_capacity(num_hands);
    ...
    // player's own node — update regrets and strategy
        ...
        // compute strategy BEFORE recursion (needed to split player_reach)
        let mut strategy = if game.is_compression_enabled() {
            regret_matching_compressed(node.regrets_compressed(), num_actions)
        } else {
            regret_matching(node.regrets(), num_actions)
        ...
        // update cumulative strategy and regrets — identical to solve_recursive()
        if game.is_compression_enabled() {
            let scale = node.strategy_scale();
            let decoder = params.gamma_t * scale / u16::MAX as f32;
            let cum_strategy = node.strategy_compressed_mut();
