...
/// Deepstack-aware finalize: computes and saves CFVs using oracle at turn chance nodes.
#[cfg(feature = "onnx")]
pub(crate) fn finalize_deepstack(
    ...
    for player in 0..2 {
        let mut cfvalues = Vec::with_capacity(game.num_private_hands(player));
        compute_cfvalue_recursive_deepstack(
            cfvalues.spare_capacity_mut(),
            game,
            &mut game.root(),
            player,
...
/// Deepstack-aware CFV computation with save support.
/// Same as `compute_cfvalue_recursive` but intercepts at turn chance nodes
/// and uses the oracle instead of recursing into turn+river subtrees.
#[cfg(feature = "onnx")]
fn compute_cfvalue_recursive_deepstack(
    ...
    // chance node (river chance — turn already dealt)
    if node.is_chance() {
...
/// Computes exploitability using oracle at turn chance nodes instead of full tree traversal.
#[cfg(feature = "onnx")]
pub(crate) fn compute_exploitability_deepstack(
    game: &PostFlopGame,
    oracle: &oracle::OnnxOracle,
    combo_to_hand: &[Vec<usize>; 2],
) -> f32 {
    let mes_ev = compute_mes_ev_deepstack(game, oracle, combo_to_hand);
    (mes_ev[0] + mes_ev[1]) * 0.5
}

/// Computes the MES (Maximally Exploitative Strategy) EVs using oracle at turn chance nodes.
#[cfg(feature = "onnx")]
fn compute_mes_ev_deepstack(
    game: &PostFlopGame,
    oracle: &oracle::OnnxOracle,
    combo_to_hand: &[Vec<usize>; 2],
) -> [f32; 2] {
    ...
    for player in 0..2 {
        compute_best_cfv_recursive_deepstack(
            cfvalues[player].spare_capacity_mut(),
            game,
            &game.root(),
            player,
...
fn compute_best_cfv_recursive<T: Game>(
    ...
    let num_actions = node.num_actions();
    let num_hands = result.len(); // Use result.len() like compute_cfvalue_recursive does
...
/// Deepstack-aware best-response CFV computation.
/// Same as `compute_best_cfv_recursive` but intercepts at turn chance nodes
/// and uses the oracle instead of recursing into turn+river subtrees.
#[cfg(feature = "onnx")]
fn compute_best_cfv_recursive_deepstack(
    result: &mut [MaybeUninit<f32>],
    game: &PostFlopGame,
    node: &PostFlopNode,
    player: usize,
    ...
    if num_actions == 1 && !node.is_chance() {
        let child = &node.play(0);
        compute_best_cfv_recursive_deepstack(
            result, game, child, player, cfreach, oracle, combo_to_hand,
        );
        return;
    }
    ...
    // player node: take best response (MAX over actions)
    else if node.player() == player {
        for_each_child(node, |action| {
            compute_best_cfv_recursive_deepstack(
                row_mut(cfv_actions.lock().spare_capacity_mut(), action, num_hands),
